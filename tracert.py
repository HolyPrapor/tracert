...
def main(args):
    ...
    for counter, i in enumerate(icmp.get_trace(args.interface_ip, args.dest_ip, args.depth, 2)):
        to_print = [str(counter + 1), '. ', i, '\r\n']
        if i != 'BAD':
            if whois.is_ip_local(i):
                to_print.append('local')
            else:
                data = whois.get_whois_info(i)
